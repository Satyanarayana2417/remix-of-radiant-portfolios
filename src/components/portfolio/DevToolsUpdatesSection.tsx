    ...
    <section className="container-x py-10 md:py-12">
      {showHint && (
        <div className="mb-8 rounded-2xl border border-border bg-surface p-4 text-sm text-foreground md:flex md:items-center md:justify-between md:gap-4">
          <p>
            Press{" "}
            <kbd className="rounded bg-secondary px-1.5 py-0.5 font-mono text-xs">Ctrl + I</kbd> to
            turn on code suggestions. Press{" "}
            <kbd className="rounded bg-secondary px-1.5 py-0.5 font-mono text-xs">Ctrl + X</kbd> to
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {updates.map((update) => (
          <article key={update.title} className="rounded-2xl border border-border bg-surface p-5">
